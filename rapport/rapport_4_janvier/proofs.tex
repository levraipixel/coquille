...
\coqcode{Cpser} est une généralisation de \coqcode{Rpser} et doit être conçu de manière plus modulaire : le but n'est plus seulement de faire tomber des résultats théoriques majeurs mais de fournir également des lemmes intermédiaires réutilisables lorsqu'on voudra s'intéresser à des cas particuliers.

\subsubsection{Description}

Cette bibliothèque commence par établir des liens entre les séries entières réelles et les séries entières complexes afin de simplifier les preuves de convergence. On utilsera également un résultat majeur de \coqcode{Csequence} qui prouve que si $(z_n)_{n \in \mathbb{N}}$ vérifie le critère de Cauchy, alors $(Im (z_n))_{n \in \mathbb{N}}$ et $(Re(z_n))_{n \in \mathbb{N}}$ le vérifient également (et réciproquement).
